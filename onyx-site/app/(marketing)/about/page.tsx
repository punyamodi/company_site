      ...
      <section className="section" style={{ paddingTop: 80 }}>
        ...
        <div className="container" style={{ position: "relative" }}>
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
            <div className="eyebrow">About Onyx</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64 }}>
            <div>
              ...
              <p style={{ fontSize: 16, color: "var(--nb-ink-muted)", lineHeight: 1.75, marginBottom: 20 }}>
                Onyx is the AI platform that connects to your organization's knowledge sources and makes them accessible through natural conversation — with citations, access control, and the depth of analysis that complex decisions require.
              </p>
              <p style={{ fontSize: 16, color: "var(--nb-ink-muted)", lineHeight: 1.75 }}>
                We believe that AI in the enterprise must be trustworthy: grounded in real documents, controlled by real permissions, and auditable by real compliance teams. We built Onyx to hold that standard.
              </p>
            </div>
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                {[
                  { icon: Target, title: "Grounded in real knowledge", desc: "Onyx never fabricates. Every response is grounded in retrieved documents with cited sources, so users can trust what they read and trace where it came from." },
                  { icon: Shield, title: "Security is the foundation", desc: "Access control, audit logging, tenant isolation, and deployment flexibility are not optional features — they are fundamental to how Onyx works." },
                  { icon: Zap, title: "Agentic where it matters", desc: "For complex research and multi-step workflows, Onyx agents reason, plan, and synthesize at a depth that simple search cannot match." },
                  { icon: Globe, title: "LLM-agnostic by design", desc: "Organizations should not be locked to a single AI provider. Onyx works with every major LLM and supports on-premises models for complete independence." },
                ].map((v) => {
                  const Icon = v.icon;
                  return (
